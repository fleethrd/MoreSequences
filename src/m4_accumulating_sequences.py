...
def run_test_make_less_simple_string():
    ...
    # Test 2 (add your test here):
    expected = '7-8-9-10-11-12'
    actual = make_less_simple_string(7, 12)
    print('Expected:', expected)
    print('Actual:  ', actual)
...
def make_less_simple_string(m, n):
    ...
    return make_simple_string(m, n - 1) + str(n)
...
def rectangles_from_circles(circles):
    ...
    for k in range(len(circles)):
        rectangle = rg.Rectangle(rg.Point(circles[k].center.x - circles[
            k].radius, circles[k].center.y - circles[k].radius), rg.Point(
            circles[k].center.x + circles[
                k].radius, circles[k].center.y + circles[k].radius))
